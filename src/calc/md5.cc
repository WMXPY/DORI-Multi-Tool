...
void MD5(const Nan::FunctionCallbackInfo<Value> &info)
{
    ...
    auto message = Nan::New(md5_str).ToLocalChecked();
...
void MD5Encode(unsigned char *output, unsigned int *input, unsigned int length)
{
    unsigned int i, j;
...
void MD5Decode(unsigned int *output, unsigned char *input, unsigned int length)
{
    unsigned int i, j;
